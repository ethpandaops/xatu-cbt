...
func BuildListFctMevBidHighestValueByBuilderChunked50MsQuery(req *ListFctMevBidHighestValueByBuilderChunked50MsRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "chunk_slot_start_diff", "toUnixTimestamp64Micro(`earliest_bid_date_time`) AS `earliest_bid_date_time`", "relay_names", "NULLIF(`block_hash`, repeat('\x00', 66)) AS `block_hash`", "builder_pubkey", "toString(`value`) AS `value`"}
