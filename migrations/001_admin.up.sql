CREATE TABLE IF NOT EXISTS admin_cbt_incremental_local ON CLUSTER '{cluster}' (
...
CREATE TABLE IF NOT EXISTS admin_cbt_incremental ON CLUSTER '{cluster}' AS admin_cbt_incremental_local
ENGINE = Distributed(
    '{cluster}',
    currentDatabase(),
...
CREATE TABLE IF NOT EXISTS admin_cbt_scheduled_local ON CLUSTER '{cluster}' (
    updated_date_time DateTime(3) CODEC(DoubleDelta, ZSTD(1)),
    database LowCardinality(String) COMMENT 'The database name',
    table LowCardinality(String) COMMENT 'The table name',
...
CREATE TABLE IF NOT EXISTS admin_cbt_scheduled ON CLUSTER '{cluster}' AS admin_cbt_scheduled_local
ENGINE = Distributed(
    '{cluster}',
    currentDatabase(),
