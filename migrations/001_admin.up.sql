CREATE TABLE IF NOT EXISTS ${NETWORK_NAME}.cbt_incremental_local ON CLUSTER '{cluster}' (
...
CREATE TABLE IF NOT EXISTS ${NETWORK_NAME}.cbt_incremental ON CLUSTER '{cluster}' AS ${NETWORK_NAME}.cbt_incremental_local
ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
...
CREATE TABLE IF NOT EXISTS ${NETWORK_NAME}.cbt_scheduled_local ON CLUSTER '{cluster}' (
    updated_date_time DateTime(3) CODEC(DoubleDelta, ZSTD(1)),
    database LowCardinality(String) COMMENT 'The database name',
    table LowCardinality(String) COMMENT 'The table name',
...
CREATE TABLE IF NOT EXISTS ${NETWORK_NAME}.cbt_scheduled ON CLUSTER '{cluster}' AS ${NETWORK_NAME}.cbt_scheduled_local
ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    'cbt_scheduled_local',
