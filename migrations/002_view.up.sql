...
CREATE VIEW `${NETWORK_NAME}`.beacon_api_eth_v1_beacon_committee ON CLUSTER '{cluster}' AS
SELECT
    *
FROM
...
CREATE VIEW `${NETWORK_NAME}`.canonical_beacon_blob_sidecar ON CLUSTER '{cluster}' AS
SELECT
    *
FROM
...
CREATE VIEW `${NETWORK_NAME}`.mempool_transaction ON CLUSTER '{cluster}' AS
SELECT
    *
FROM
