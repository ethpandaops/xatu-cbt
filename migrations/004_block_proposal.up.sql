CREATE TABLE `${NETWORK_NAME}`.int_block_proposers_head_local on cluster '{cluster}' (
...
CREATE TABLE `${NETWORK_NAME}`.int_block_proposers_head ON CLUSTER '{cluster}' AS `${NETWORK_NAME}`.int_block_proposers_head_local ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    int_block_proposers_head_local,
    cityHash64(`slot_start_date_time`, `proposer_validator_index`)
);

CREATE TABLE `${NETWORK_NAME}`.int_block_proposers_canonical_local on cluster '{cluster}' (
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number for which the proposer duty is assigned' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
    `epoch` UInt32 COMMENT 'The epoch number containing the slot' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE `${NETWORK_NAME}`.int_block_proposers_canonical ON CLUSTER '{cluster}' AS `${NETWORK_NAME}`.int_block_proposers_canonical_local ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    int_block_proposers_canonical_local,
