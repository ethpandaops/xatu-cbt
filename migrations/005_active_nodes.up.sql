CREATE TABLE `${NETWORK_NAME}`.int_nodes_active_last_24h_local on cluster '{cluster}' (
...
CREATE TABLE `${NETWORK_NAME}`.int_nodes_active_last_24h ON CLUSTER '{cluster}' AS `${NETWORK_NAME}`.int_nodes_active_last_24h_local ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    int_nodes_active_last_24h_local,
