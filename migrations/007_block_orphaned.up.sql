CREATE TABLE `${NETWORK_NAME}`.int_block_orphaned_local on cluster '{cluster}' (
...
CREATE TABLE `${NETWORK_NAME}`.int_block_orphaned ON CLUSTER '{cluster}' AS `${NETWORK_NAME}`.int_block_orphaned_local ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    int_block_orphaned_local,
