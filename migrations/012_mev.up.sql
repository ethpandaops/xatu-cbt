CREATE TABLE `${NETWORK_NAME}`.int_block_mev_head_local on cluster '{cluster}' (
    ...
    `bid_date_time` Nullable(DateTime64(3)) COMMENT 'The timestamp of the earliest bid trace in milliseconds' CODEC(ZSTD(1)),
...
CREATE TABLE `${NETWORK_NAME}`.fct_mev_bid_highest_value_by_relay_local on cluster '{cluster}' (
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    ...
    `parent_hash` FixedString(66) COMMENT 'The parent block hash of the execution block' CODEC(ZSTD(1)),
    `block_number` UInt64 COMMENT 'The execution block number of the bid' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE `${NETWORK_NAME}`.fct_mev_bid_highest_value_by_relay ON CLUSTER '{cluster}' AS `${NETWORK_NAME}`.fct_mev_bid_highest_value_by_relay_local ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    fct_mev_bid_highest_value_by_relay_local,
    cityHash64(`slot_start_date_time`, `relay_name`)
);

ALTER TABLE `${NETWORK_NAME}`.fct_mev_bid_highest_value_by_relay_local
ADD PROJECTION p_by_slot
(
    SELECT *
...
(
    SELECT *
    ORDER BY (`block_hash`, `slot`, `relay_name`)
);
