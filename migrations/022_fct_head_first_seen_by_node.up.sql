CREATE TABLE `${NETWORK_NAME}`.fct_head_first_seen_by_node_local on cluster '{cluster}' (
...
CREATE TABLE `${NETWORK_NAME}`.fct_head_first_seen_by_node ON CLUSTER '{cluster}' AS `${NETWORK_NAME}`.fct_head_first_seen_by_node_local ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    fct_head_first_seen_by_node_local,
    cityHash64(`slot_start_date_time`, `meta_client_name`)
);

ALTER TABLE `${NETWORK_NAME}`.fct_head_first_seen_by_node_local ON CLUSTER '{cluster}'
