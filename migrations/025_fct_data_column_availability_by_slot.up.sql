CREATE TABLE fct_data_column_availability_by_slot_local on cluster '{cluster}' (
...
CREATE TABLE fct_data_column_availability_by_slot ON CLUSTER '{cluster}' AS fct_data_column_availability_by_slot_local ENGINE = Distributed(
    '{cluster}',
    currentDatabase(),
    fct_data_column_availability_by_slot_local,
    cityHash64(`slot_start_date_time`, `column_index`)
);

ALTER TABLE fct_data_column_availability_by_slot_local ON CLUSTER '{cluster}'
ADD PROJECTION p_by_slot_column
(
    SELECT *
