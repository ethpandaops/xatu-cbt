-- Revert ORDER BY clause change back to original (including status)
-- This reverts the fix that prevents duplicate rows
...
-- Recreate local table with original ORDER BY (including status)
