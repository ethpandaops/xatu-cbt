...
CREATE TABLE `${NETWORK_NAME}`.fct_execution_state_size_hourly_local ON CLUSTER '{cluster}' (
    ...
    `account_count` UInt64 COMMENT 'Total accounts at end of hour' CODEC(ZSTD(1)),
    ...
    `storage_count` UInt64 COMMENT 'Storage slots at end of hour' CODEC(ZSTD(1)),
...
CREATE TABLE `${NETWORK_NAME}`.fct_execution_state_size_daily_local ON CLUSTER '{cluster}' (
    ...
    `account_count` UInt64 COMMENT 'Total accounts at end of day' CODEC(ZSTD(1)),
    ...
    `storage_count` UInt64 COMMENT 'Storage slots at end of day' CODEC(ZSTD(1)),
...
CREATE TABLE `${NETWORK_NAME}`.fct_execution_state_size_monthly_local ON CLUSTER '{cluster}' (
    ...
    `account_count` UInt64 COMMENT 'Total accounts at end of month' CODEC(ZSTD(1)),
    ...
    `storage_count` UInt64 COMMENT 'Storage slots at end of month' CODEC(ZSTD(1)),
