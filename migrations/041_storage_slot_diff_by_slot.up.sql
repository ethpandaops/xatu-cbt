CREATE TABLE `${NETWORK_NAME}`.int_storage_slot_diff_by_slot_local ON CLUSTER '{cluster}' (
...
COMMENT 'Storage slot diffs ordered by (address, slot_key, block_number) for efficient per-slot range queries';

CREATE TABLE `${NETWORK_NAME}`.int_storage_slot_diff_by_slot ON CLUSTER '{cluster}' AS `${NETWORK_NAME}`.int_storage_slot_diff_by_slot_local ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    int_storage_slot_diff_by_slot_local,
