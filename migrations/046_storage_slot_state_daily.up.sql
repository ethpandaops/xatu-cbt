...
CREATE TABLE `${NETWORK_NAME}`.fct_storage_slot_state_daily_local ON CLUSTER '{cluster}' (
    ...
    `active_slots_with_6m_expiry` Int64 COMMENT 'Cumulative count of active storage slots at end of day (with 6-month expiry applied)' CODEC(ZSTD(1)),
    `effective_bytes_with_6m_expiry` Int64 COMMENT 'Cumulative sum of effective bytes at end of day (with 6-month expiry applied)' CODEC(ZSTD(1))
