...
CREATE TABLE `${NETWORK_NAME}`.fct_attestation_participation_rate_hourly_local ON CLUSTER '{cluster}' (
...
CREATE TABLE `${NETWORK_NAME}`.fct_attestation_participation_rate_hourly ON CLUSTER '{cluster}'
AS `${NETWORK_NAME}`.fct_attestation_participation_rate_hourly_local
ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    fct_attestation_participation_rate_hourly_local,
    cityHash64(hour_start_date_time)
);

-- fct_attestation_participation_rate_daily
CREATE TABLE `${NETWORK_NAME}`.fct_attestation_participation_rate_daily_local ON CLUSTER '{cluster}' (
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE `${NETWORK_NAME}`.fct_attestation_participation_rate_daily ON CLUSTER '{cluster}'
AS `${NETWORK_NAME}`.fct_attestation_participation_rate_daily_local
ENGINE = Distributed(
    '{cluster}',
    '${NETWORK_NAME}',
    fct_attestation_participation_rate_daily_local,
