...
CREATE TABLE fct_blob_count_hourly_local ON CLUSTER '{cluster}' (
...
CREATE TABLE fct_blob_count_hourly ON CLUSTER '{cluster}'
AS fct_blob_count_hourly_local
ENGINE = Distributed(
    '{cluster}',
    currentDatabase(),
...
CREATE TABLE fct_blob_count_daily_local ON CLUSTER '{cluster}' (
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE fct_blob_count_daily ON CLUSTER '{cluster}'
AS fct_blob_count_daily_local
ENGINE = Distributed(
    '{cluster}',
    currentDatabase(),
