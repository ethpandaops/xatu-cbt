-- fct_reorg_hourly
CREATE TABLE fct_reorg_hourly_local ON CLUSTER '{cluster}' (
...
CREATE TABLE fct_reorg_hourly ON CLUSTER '{cluster}'
AS fct_reorg_hourly_local
ENGINE = Distributed(
    '{cluster}',
    currentDatabase(),
...
-- fct_reorg_daily
CREATE TABLE fct_reorg_daily_local ON CLUSTER '{cluster}' (
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE fct_reorg_daily ON CLUSTER '{cluster}'
AS fct_reorg_daily_local
ENGINE = Distributed(
    '{cluster}',
    currentDatabase(),
