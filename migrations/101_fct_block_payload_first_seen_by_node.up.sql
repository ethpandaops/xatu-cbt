CREATE TABLE fct_block_payload_first_seen_by_node_local on cluster '{cluster}' (
    ...
    `seen_slot_start_diff` UInt32 COMMENT 'The time from slot start for the client to see the execution payload envelope' CODEC(DoubleDelta, ZSTD(1)),
    `block_root` String COMMENT 'The beacon block root the envelope references' CODEC(ZSTD(1)),
    `block_hash` FixedString(66) COMMENT 'The execution block hash of the payload' CODEC(ZSTD(1)),
    `builder_index` UInt64 COMMENT 'Index of the builder that produced the payload' CODEC(DoubleDelta, ZSTD(1)),
...
COMMENT 'When the gloas (ePBS) execution payload envelope was first seen by each sentry node, across the beacon API event stream and libp2p gossip';

CREATE TABLE fct_block_payload_first_seen_by_node ON CLUSTER '{cluster}' AS fct_block_payload_first_seen_by_node_local ENGINE = Distributed(
    '{cluster}',
    currentDatabase(),
    fct_block_payload_first_seen_by_node_local,
