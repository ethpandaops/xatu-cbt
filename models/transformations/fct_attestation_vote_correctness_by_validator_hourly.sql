...
WITH
    ...
    -- Committee duties for all slots within the hour boundaries
    duties AS (
        SELECT
            slot,
    ...
    -- Attestation data with source/target roots for all slots in hour boundaries
    attestations AS (
        SELECT
            slot,
            slot_start_date_time,
    ...
    blocks AS (
        ...
        WHERE slot_start_date_time >= (SELECT min_hour FROM hour_bounds) - INTERVAL '768 SECOND'
          AND slot_start_date_time < (SELECT max_hour FROM hour_bounds) + INTERVAL 1 HOUR
    ),
    ...
    epoch_blocks AS (
        ...
        WHERE slot_start_date_time >= (SELECT min_hour FROM hour_bounds) - INTERVAL '1 DAY'
          AND slot_start_date_time < (SELECT max_hour FROM hour_bounds) + INTERVAL 1 HOUR
          AND meta_network_name = '{{ .env.NETWORK }}'
        GROUP BY epoch
    ),
    ...
    finalized_checkpoints AS (
        ...
        WHERE epoch_start_date_time >= (SELECT min_hour FROM hour_bounds) - INTERVAL '1 DAY'
