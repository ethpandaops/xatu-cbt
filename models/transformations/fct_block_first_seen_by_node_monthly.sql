...
FROM (
    SELECT
    toYYYYMM(toDate(slot_start_date_time)) as month,
    toDateTime(toStartOfMonth(toDate(slot_start_date_time))) as month_start_date_time,
    username,
    node_id,
    classification,
    meta_client_name,
    ...
    countDistinct(toDate(slot_start_date_time)) AS day_count,
