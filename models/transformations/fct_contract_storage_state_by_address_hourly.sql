---
table: fct_contract_storage_state_by_address_hourly
type: incremental
interval:
  type: block
  max: 1000
...
tags:
  ...
  - contract
dependencies:
  - "{{transformation}}.int_contract_storage_state_by_address"
...
FROM (
    ...
    FROM (
        SELECT
            ...
            s.effective_bytes,
            b.block_date_time
        FROM {{ index .dep "{{transformation}}" "int_contract_storage_state_by_address" "helpers" "from" }} AS s FINAL
        GLOBAL INNER JOIN blocks_in_hours AS b ON s.block_number = b.block_number
    )
    GROUP BY address, hour_start_date_time
)
