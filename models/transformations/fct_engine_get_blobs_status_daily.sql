...
FROM (
    SELECT
        ...
        ifNotFinite(round(avgIf(slots_in_days.avg_duration_ms, status = 'SUCCESS')), 0) AS avg_duration_ms,
        ifNotFinite(round(avgIf(slots_in_days.median_duration_ms, status = 'SUCCESS')), 0) AS avg_p50_duration_ms,
        ifNotFinite(round(avgIf(slots_in_days.p95_duration_ms, status = 'SUCCESS')), 0) AS avg_p95_duration_ms,
        ifNotFinite(maxIf(slots_in_days.max_duration_ms, status = 'SUCCESS'), 0) AS max_duration_ms
    FROM slots_in_days
