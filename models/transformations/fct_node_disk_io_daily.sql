...
tags:
  ...
  - disk
dependencies:
  - "{{transformation}}.fct_node_disk_io_hourly"
---
-- Daily aggregation of node disk I/O from hourly data.
...
WITH
    day_bounds AS (
        ...
        FROM {{ index .dep "{{transformation}}" "fct_node_disk_io_hourly" "helpers" "from" }} FINAL
...
SELECT
    ...
    sum(sum_io_ops) AS sum_io_ops,
    toUInt32(round(sum(avg_io_ops * slot_count) / sum(slot_count))) AS avg_io_ops
FROM hours_in_days
GROUP BY toDate(hour_start_date_time), meta_client_name, rw
