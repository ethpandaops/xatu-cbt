---
table: fct_node_network_io_hourly
type: incremental
interval:
  type: slot
  max: 25200
...
tags:
  ...
  - network
dependencies:
  - "{{transformation}}.fct_node_network_io_by_process"
---
-- Hourly aggregation of node network I/O.
-- Sums across processes per (slot, node, port_label, direction) to get total node network I/O,
-- then aggregates into hourly stats.
...
WITH
    hour_bounds AS (
        ...
        FROM {{ index .dep "{{transformation}}" "fct_node_network_io_by_process" "helpers" "from" }} FINAL
    ...
    slot_node_totals AS (
        ...
        GROUP BY wallclock_slot_start_date_time, meta_client_name, port_label, direction
    )
SELECT
    ...
    sum(total_io_count) AS sum_io_count,
    toUInt32(round(avg(total_io_count))) AS avg_io_count
FROM slot_node_totals
GROUP BY toStartOfHour(wallclock_slot_start_date_time), meta_client_name, port_label, direction
