---
table: fct_storage_slot_state_with_expiry_by_address_hourly
type: incremental
interval:
  type: block
  max: 1000
...
FROM (
    ...
    FROM (
        SELECT
            ...
            s.effective_bytes,
            b.block_date_time
        FROM {{ index .dep "{{transformation}}" "int_storage_slot_state_with_expiry_by_address" "helpers" "from" }} AS s FINAL
        GLOBAL INNER JOIN blocks_in_hours AS b ON s.block_number = b.block_number
    )
    GROUP BY address, expiry_policy, hour_start_date_time
)
