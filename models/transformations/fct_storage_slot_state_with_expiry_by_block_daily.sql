---
table: fct_storage_slot_state_with_expiry_by_block_daily
type: incremental
interval:
  type: block
  max: 1000
...
tags:
  ...
  - storage
dependencies:
  - "{{transformation}}.int_storage_slot_state_with_expiry_by_block"
...
FROM (
    ...
    FROM (
        SELECT
            ...
            s.effective_bytes,
            b.block_date_time
        FROM {{ index .dep "{{transformation}}" "int_storage_slot_state_with_expiry_by_block" "helpers" "from" }} AS s FINAL
        GLOBAL INNER JOIN blocks_in_days AS b ON s.block_number = b.block_number
    )
    GROUP BY expiry_policy, day_start_date
)
