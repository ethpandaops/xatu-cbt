---
table: fct_transaction_mempool_outcome_daily
type: incremental
interval:
  type: slot
  max: 604800
...
---
-- Daily cohort outcomes for transactions first sighted in the public mempool,
...
-- complete day boundaries so partial boundary days get re-aggregated and replaced.
INSERT INTO `{{ .self.database }}`.`{{ .self.table }}`
WITH outcomes AS (
    SELECT
        toDate(first_seen_date_time) AS day_start_date,
    ...
    WHERE first_seen_date_time BETWEEN fromUnixTimestamp({{ .bounds.start }}) - INTERVAL 25 HOUR
        AND fromUnixTimestamp({{ .bounds.end }}) + INTERVAL 25 HOUR
        AND toDate(first_seen_date_time) >= toDate(fromUnixTimestamp({{ .bounds.start }}))
        AND toDate(first_seen_date_time) <= toDate(fromUnixTimestamp({{ .bounds.end }}))
    GROUP BY day_start_date
),
replacements AS (
    SELECT
        toDate(group_first_seen_date_time) AS day_start_date,
    ...
    WHERE group_first_seen_date_time BETWEEN fromUnixTimestamp({{ .bounds.start }}) - INTERVAL 25 HOUR
        AND fromUnixTimestamp({{ .bounds.end }}) + INTERVAL 25 HOUR
        AND toDate(group_first_seen_date_time) >= toDate(fromUnixTimestamp({{ .bounds.start }}))
        AND toDate(group_first_seen_date_time) <= toDate(fromUnixTimestamp({{ .bounds.end }}))
    GROUP BY day_start_date
)
SELECT
    fromUnixTimestamp({{ .task.start }}) AS updated_date_time,
    o.day_start_date AS day_start_date,
...
FROM outcomes o
GLOBAL LEFT JOIN replacements r ON r.day_start_date = o.day_start_date
