---
table: int_contract_storage_reactivation_12m
type: incremental
interval:
  type: block
  max: 1000
...
---
-- 12-month reactivation: Detects when a contract is touched after its 12m expiry event.
INSERT INTO
...
-- Pre-filter 12m expiry table using IN clause
filtered_expiry AS (
    SELECT
        address,
        block_number,
    ...
    FROM {{ index .dep "{{transformation}}" "int_contract_storage_expiry_12m" "helpers" "from" }}
