---
table: int_contract_storage_reactivation_18m
type: incremental
interval:
  type: block
  max: 1000
...
---
-- 18-month reactivation: Detects when a contract is touched after its 18m expiry event.
INSERT INTO
...
-- Pre-filter 18m expiry table using IN clause
filtered_expiry AS (
    SELECT
        address,
        block_number,
    ...
    FROM {{ index .dep "{{transformation}}" "int_contract_storage_expiry_18m" "helpers" "from" }}
