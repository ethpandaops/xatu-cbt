---
table: int_contract_storage_reactivation_1m
type: incremental
interval:
  type: block
  max: 1000
...
---
-- 1-month reactivation: Detects when a contract is touched after its 1m expiry event.
-- A reactivation occurs when next_touch_block falls within current bounds
-- and there was a 1m expiry between the previous touch and the reactivation.
-- This is the contract-level equivalent of int_storage_slot_reactivation_1m.
INSERT INTO
...
-- Max block that is 1+ month before current bounds (minimum expiry period)
old_block_range AS (
    SELECT max(block_number) as max_old_block
    FROM {{ index .dep "{{transformation}}" "int_execution_block_by_date" "helpers" "from" }} FINAL
    WHERE block_date_time <= (SELECT max_time - INTERVAL 1 MONTH + INTERVAL 1 DAY FROM current_bounds)
),
-- Reactivation candidates: previous touch was > 1 month ago
...
-- Pre-filter 1m expiry table using IN clause, dedupe via argMax (avoids FINAL overhead)
filtered_expiry AS (
    SELECT
        address,
        block_number,
    ...
    FROM {{ index .dep "{{transformation}}" "int_contract_storage_expiry_1m" "helpers" "from" }}
...
-- Join with filtered expiry table to get active_slots/effective_bytes from the expiry record
-- Uses argMax to get the most recent expiry's values if multiple exist
SELECT
