---
table: int_storage_slot_expiry_12m
type: incremental
interval:
  type: block
  max: 1000
...
---
-- 12-month expiry tier: Waterfalls from 6m expiries.
-- Scans 6 months of 6m expiries.
-- A slot expires at 12m if it was 6m-expired 6 months ago AND not reactivated before 12m expiry.
-- Checks int_storage_slot_reactivation_6m instead of relying on propagated next_touch_block.
INSERT INTO
...
-- Get 6-month-ago block range (for 6m expiries that are now 12m old)
...
-- Get 6m expiries from 6 months ago (these are candidates for 12m expiry)
source_expiries AS (
    SELECT
...
-- Get 6m reactivations that could invalidate these 12m expiries
-- A reactivation invalidates if it happened before the new 12m expiry block
...
WHERE
    -- No reactivation before 12m expiry block
