---
table: int_storage_slot_expiry_18m
type: incremental
interval:
  type: block
  max: 1000
...
---
-- 18-month expiry tier: Waterfalls from 12m expiries.
-- Scans 6 months of 12m expiries.
-- A slot expires at 18m if it was 12m-expired 6 months ago AND not reactivated before 18m expiry.
-- Checks int_storage_slot_reactivation_12m instead of relying on propagated next_touch_block.
INSERT INTO
...
-- Get 6-month-ago block range (for 12m expiries that are now 18m old)
...
-- Get 12m expiries from 6 months ago (these are candidates for 18m expiry)
source_expiries AS (
    SELECT
...
-- Get 12m reactivations that could invalidate these 18m expiries
-- A reactivation invalidates if it happened before the new 18m expiry block
...
WHERE
    -- No reactivation before 18m expiry block
