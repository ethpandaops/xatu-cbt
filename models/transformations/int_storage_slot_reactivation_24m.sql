---
table: int_storage_slot_reactivation_24m
type: incremental
interval:
  type: block
  max: 100000
...
---
-- 24-month reactivation: Detects when a slot is touched after its 24m expiry event.
-- A reactivation occurs when next_touch_block falls within current bounds
-- and there was a 24m expiry between the previous touch and the reactivation.
INSERT INTO
...
-- Max block that is 24+ months before current bounds
old_block_range AS (
    SELECT max(block_number) as max_old_block
    FROM {{ index .dep "{{transformation}}" "int_execution_block_by_date" "helpers" "from" }} FINAL
    WHERE block_date_time <= (SELECT max_time - INTERVAL 24 MONTH + INTERVAL 1 DAY FROM current_bounds)
),
-- Reactivation candidates: previous touch was > 24 months ago
...
-- Pre-filter 24m expiry table using IN clause, dedupe via argMax (avoids FINAL overhead)
filtered_expiry AS (
    SELECT
        address,
        slot_key,
    ...
    FROM {{ index .dep "{{transformation}}" "int_storage_slot_expiry_24m" "helpers" "from" }}
