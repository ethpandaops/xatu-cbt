---
table: int_storage_slot_reactivation_6m
type: incremental
interval:
  type: block
  max: 1000
...
---
-- 6-month reactivation: Detects when a slot is touched after its 6m expiry event.
...
-- Pre-filter 6m expiry table using IN clause, dedupe via argMax (avoids FINAL overhead)
filtered_expiry AS (
    SELECT
        address,
        slot_key,
    ...
    FROM {{ index .dep "{{transformation}}" "int_storage_slot_expiry_6m" "helpers" "from" }}
