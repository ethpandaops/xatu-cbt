...
-- Uses projection on next_touch_block for efficient filtering (no FINAL - dedupes via argMax).
INSERT INTO
...
-- Pre-filter expiry table using IN clause, dedupe via argMax (avoids FINAL overhead)
filtered_expiry AS (
    SELECT
        address,
        slot_key,
    ...
    FROM {{ index .dep "{{transformation}}" "int_storage_slot_expiry_by_6m" "helpers" "from" }}
    ...
    GROUP BY address, slot_key, block_number
)
