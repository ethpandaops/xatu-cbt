...
// SQL query builder for fct_address_storage_slot_expired_top_100_by_contract
...
// BuildListFctAddressStorageSlotExpiredTop100ByContractQuery constructs a parameterized SQL query from a ListFctAddressStorageSlotExpiredTop100ByContractRequest
func BuildListFctAddressStorageSlotExpiredTop100ByContractQuery(req *ListFctAddressStorageSlotExpiredTop100ByContractRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: expired_slots
	if req.ExpiredSlots != nil {
		switch filter := req.ExpiredSlots.Filter.(type) {
		case *UInt64Filter_Eq:
			qb.AddCondition("expired_slots", "=", filter.Eq)
		case *UInt64Filter_Ne:
			qb.AddCondition("expired_slots", "!=", filter.Ne)
		case *UInt64Filter_Lt:
			qb.AddCondition("expired_slots", "<", filter.Lt)
		case *UInt64Filter_Lte:
			qb.AddCondition("expired_slots", "<=", filter.Lte)
		case *UInt64Filter_Gt:
			qb.AddCondition("expired_slots", ">", filter.Gt)
		case *UInt64Filter_Gte:
			qb.AddCondition("expired_slots", ">=", filter.Gte)
		case *UInt64Filter_Between:
			qb.AddBetweenCondition("expired_slots", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt64Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("expired_slots", UInt64SliceToInterface(filter.In.Values))
			}
		case *UInt64Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("expired_slots", UInt64SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "rank", "contract_address", "expired_slots"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "rank", "contract_address", "expired_slots"}

	return BuildParameterizedQuery("fct_address_storage_slot_expired_top_100_by_contract", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctAddressStorageSlotExpiredTop100ByContractQuery constructs a parameterized SQL query from a GetFctAddressStorageSlotExpiredTop100ByContractRequest
func BuildGetFctAddressStorageSlotExpiredTop100ByContractQuery(req *GetFctAddressStorageSlotExpiredTop100ByContractRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_address_storage_slot_expired_top_100_by_contract", columns, qb, orderByClause, 1, 0, options...)
}
