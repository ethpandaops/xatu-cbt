...
// SQL query builder for fct_attestation_liveness_by_entity_head_epoch
...
// BuildListFctAttestationLivenessByEntityHeadEpochQuery constructs a parameterized SQL query from a ListFctAttestationLivenessByEntityHeadEpochRequest
//
// Available projections:
//   - p_by_epoch (primary key: epoch)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctAttestationLivenessByEntityHeadEpochQuery(req *ListFctAttestationLivenessByEntityHeadEpochRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "epoch", "epoch_start_date_time", "entity", "status", "attestation_count"}
		...
		orderByClause = " ORDER BY epoch_start_date_time" + ", entity" + ", status"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "entity", "status", "attestation_count"}

	return BuildParameterizedQuery("fct_attestation_liveness_by_entity_head_epoch", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctAttestationLivenessByEntityHeadEpochQuery constructs a parameterized SQL query from a GetFctAttestationLivenessByEntityHeadEpochRequest
func BuildGetFctAttestationLivenessByEntityHeadEpochQuery(req *GetFctAttestationLivenessByEntityHeadEpochRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_attestation_liveness_by_entity_head_epoch", columns, qb, orderByClause, 1, 0, options...)
}
