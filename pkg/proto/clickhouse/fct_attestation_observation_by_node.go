...
// SQL query builder for fct_attestation_observation_by_node
...
// BuildListFctAttestationObservationByNodeQuery constructs a parameterized SQL query from a ListFctAttestationObservationByNodeRequest
//
// Available projections:
//   - p_by_slot (primary key: slot)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctAttestationObservationByNodeQuery(req *ListFctAttestationObservationByNodeRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: avg_seen_slot_start_diff
	if req.AvgSeenSlotStartDiff != nil {
		switch filter := req.AvgSeenSlotStartDiff.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("avg_seen_slot_start_diff", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("avg_seen_slot_start_diff", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("avg_seen_slot_start_diff", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("avg_seen_slot_start_diff", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("avg_seen_slot_start_diff", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("avg_seen_slot_start_diff", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("avg_seen_slot_start_diff", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("avg_seen_slot_start_diff", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("avg_seen_slot_start_diff", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: median_seen_slot_start_diff
	if req.MedianSeenSlotStartDiff != nil {
		switch filter := req.MedianSeenSlotStartDiff.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("median_seen_slot_start_diff", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("median_seen_slot_start_diff", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("median_seen_slot_start_diff", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("median_seen_slot_start_diff", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("median_seen_slot_start_diff", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("median_seen_slot_start_diff", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("median_seen_slot_start_diff", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("median_seen_slot_start_diff", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("median_seen_slot_start_diff", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: min_seen_slot_start_diff
	if req.MinSeenSlotStartDiff != nil {
		switch filter := req.MinSeenSlotStartDiff.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("min_seen_slot_start_diff", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("min_seen_slot_start_diff", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("min_seen_slot_start_diff", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("min_seen_slot_start_diff", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("min_seen_slot_start_diff", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("min_seen_slot_start_diff", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("min_seen_slot_start_diff", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("min_seen_slot_start_diff", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("min_seen_slot_start_diff", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: max_seen_slot_start_diff
	if req.MaxSeenSlotStartDiff != nil {
		switch filter := req.MaxSeenSlotStartDiff.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("max_seen_slot_start_diff", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("max_seen_slot_start_diff", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("max_seen_slot_start_diff", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("max_seen_slot_start_diff", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("max_seen_slot_start_diff", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("max_seen_slot_start_diff", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("max_seen_slot_start_diff", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("max_seen_slot_start_diff", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("max_seen_slot_start_diff", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "slot", "slot_start_date_time", "epoch", "epoch_start_date_time", "attestation_count", "avg_seen_slot_start_diff", "median_seen_slot_start_diff", "min_seen_slot_start_diff", "max_seen_slot_start_diff", "block_root", "username", "node_id", "classification", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_consensus_version", "meta_consensus_implementation"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "attestation_count", "avg_seen_slot_start_diff", "median_seen_slot_start_diff", "min_seen_slot_start_diff", "max_seen_slot_start_diff", "block_root", "username", "node_id", "classification", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_consensus_version", "meta_consensus_implementation"}

	return BuildParameterizedQuery("fct_attestation_observation_by_node", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctAttestationObservationByNodeQuery constructs a parameterized SQL query from a GetFctAttestationObservationByNodeRequest
func BuildGetFctAttestationObservationByNodeQuery(req *GetFctAttestationObservationByNodeRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_attestation_observation_by_node", columns, qb, orderByClause, 1, 0, options...)
}
