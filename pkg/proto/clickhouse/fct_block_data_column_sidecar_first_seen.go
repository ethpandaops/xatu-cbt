...
// SQL query builder for fct_block_data_column_sidecar_first_seen
...
// BuildListFctBlockDataColumnSidecarFirstSeenQuery constructs a parameterized SQL query from a ListFctBlockDataColumnSidecarFirstSeenRequest
//
// Available projections:
//   - p_by_slot (primary key: slot)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctBlockDataColumnSidecarFirstSeenQuery(req *ListFctBlockDataColumnSidecarFirstSeenRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.ColumnIndex != nil {
		...
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("column_index", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("column_index", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		...
		orderByClause = " ORDER BY slot_start_date_time" + ", block_root" + ", column_index"
	}
	...
	return BuildParameterizedQuery("fct_block_data_column_sidecar_first_seen", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctBlockDataColumnSidecarFirstSeenQuery constructs a parameterized SQL query from a GetFctBlockDataColumnSidecarFirstSeenRequest
func BuildGetFctBlockDataColumnSidecarFirstSeenQuery(req *GetFctBlockDataColumnSidecarFirstSeenRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_block_data_column_sidecar_first_seen", columns, qb, orderByClause, 1, 0, options...)
}
