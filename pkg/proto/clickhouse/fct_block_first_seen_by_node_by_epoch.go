...
// SQL query builder for fct_block_first_seen_by_node_by_epoch
...
// BuildListFctBlockFirstSeenByNodeByEpochQuery constructs a parameterized SQL query from a ListFctBlockFirstSeenByNodeByEpochRequest
//
// Available projections:
//   - p_by_epoch_node (primary key: epoch)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctBlockFirstSeenByNodeByEpochQuery(req *ListFctBlockFirstSeenByNodeByEpochRequest, options ...QueryOption) (SQLQuery, error) {
	// Validate that at least one primary key is provided
	// Primary keys can come from base table or projections
	if req.Epoch == nil && req.EpochStartDateTime == nil {
		return SQLQuery{}, fmt.Errorf("at least one primary key field is required: epoch, epoch_start_date_time")
	}
	...
	if req.OrderBy != "" {
		...
		orderByClause = " ORDER BY epoch_start_date_time" + ", meta_client_name"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "username", "node_id", "classification", "meta_client_name", "meta_client_implementation", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "min_slot", "max_slot", "slot_count", "min_seen_slot_start_diff_ms", "p05_seen_slot_start_diff_ms", "p50_seen_slot_start_diff_ms", "avg_seen_slot_start_diff_ms", "p90_seen_slot_start_diff_ms", "p95_seen_slot_start_diff_ms", "p99_seen_slot_start_diff_ms", "max_seen_slot_start_diff_ms"}

	return BuildParameterizedQuery("fct_block_first_seen_by_node_by_epoch", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctBlockFirstSeenByNodeByEpochQuery constructs a parameterized SQL query from a GetFctBlockFirstSeenByNodeByEpochRequest
func BuildGetFctBlockFirstSeenByNodeByEpochQuery(req *GetFctBlockFirstSeenByNodeByEpochRequest, options ...QueryOption) (SQLQuery, error) {
	// Validate primary key is provided
	if req.EpochStartDateTime == 0 {
		return SQLQuery{}, fmt.Errorf("primary key field epoch_start_date_time is required")
	}
	...
	return BuildParameterizedQuery("fct_block_first_seen_by_node_by_epoch", columns, qb, orderByClause, 1, 0, options...)
}
