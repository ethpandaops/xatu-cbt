...
// SQL query builder for fct_block_first_seen_by_node_daily
...
// BuildListFctBlockFirstSeenByNodeDailyQuery constructs a parameterized SQL query from a ListFctBlockFirstSeenByNodeDailyRequest
//
// Available projections:
//   - p_by_day_node (primary key: day)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctBlockFirstSeenByNodeDailyQuery(req *ListFctBlockFirstSeenByNodeDailyRequest, options ...QueryOption) (SQLQuery, error) {
	// Validate that at least one primary key is provided
	// Primary keys can come from base table or projections
	if req.Day == nil {
		return SQLQuery{}, fmt.Errorf("primary key field day is required")
	}
	...
	switch filter := req.Day.Filter.(type) {
	case *StringFilter_Eq:
		qb.AddCondition("day", "=", filter.Eq)
	case *StringFilter_Ne:
		qb.AddCondition("day", "!=", filter.Ne)
	case *StringFilter_Contains:
		qb.AddLikeCondition("day", "%" + filter.Contains + "%")
	case *StringFilter_StartsWith:
		qb.AddLikeCondition("day", filter.StartsWith + "%")
	case *StringFilter_EndsWith:
		qb.AddLikeCondition("day", "%" + filter.EndsWith)
	case *StringFilter_Like:
		qb.AddLikeCondition("day", filter.Like)
	case *StringFilter_NotLike:
		qb.AddNotLikeCondition("day", filter.NotLike)
	case *StringFilter_In:
		if len(filter.In.Values) > 0 {
			qb.AddInCondition("day", StringSliceToInterface(filter.In.Values))
		}
	case *StringFilter_NotIn:
		if len(filter.NotIn.Values) > 0 {
			qb.AddNotInCondition("day", StringSliceToInterface(filter.NotIn.Values))
		}
	default:
	...
	// Add filter for column: day_start_date_time
	if req.DayStartDateTime != nil {
		switch filter := req.DayStartDateTime.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("day_start_date_time", "=", DateTimeValue{filter.Eq})
		case *UInt32Filter_Ne:
			qb.AddCondition("day_start_date_time", "!=", DateTimeValue{filter.Ne})
		case *UInt32Filter_Lt:
			qb.AddCondition("day_start_date_time", "<", DateTimeValue{filter.Lt})
		case *UInt32Filter_Lte:
			qb.AddCondition("day_start_date_time", "<=", DateTimeValue{filter.Lte})
		case *UInt32Filter_Gt:
			qb.AddCondition("day_start_date_time", ">", DateTimeValue{filter.Gt})
		case *UInt32Filter_Gte:
			qb.AddCondition("day_start_date_time", ">=", DateTimeValue{filter.Gte})
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("day_start_date_time", DateTimeValue{filter.Between.Min}, DateTimeValue{filter.Between.Max.GetValue()})
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				converted := make([]interface{}, len(filter.In.Values))
				for i, v := range filter.In.Values {
				...
				qb.AddInCondition("day_start_date_time", converted)
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				converted := make([]interface{}, len(filter.NotIn.Values))
				...
				qb.AddNotInCondition("day_start_date_time", converted)
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		...
		orderByClause = " ORDER BY day" + ", meta_client_name"
	}
	...
	return BuildParameterizedQuery("fct_block_first_seen_by_node_daily", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctBlockFirstSeenByNodeDailyQuery constructs a parameterized SQL query from a GetFctBlockFirstSeenByNodeDailyRequest
func BuildGetFctBlockFirstSeenByNodeDailyQuery(req *GetFctBlockFirstSeenByNodeDailyRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_block_first_seen_by_node_daily", columns, qb, orderByClause, 1, 0, options...)
}
