...
// SQL query builder for fct_block_first_seen_by_node_weekly
...
// BuildListFctBlockFirstSeenByNodeWeeklyQuery constructs a parameterized SQL query from a ListFctBlockFirstSeenByNodeWeeklyRequest
//
// Available projections:
//   - p_by_week_node (primary key: week)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctBlockFirstSeenByNodeWeeklyQuery(req *ListFctBlockFirstSeenByNodeWeeklyRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: week_start_date_time
	if req.WeekStartDateTime != nil {
		switch filter := req.WeekStartDateTime.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("week_start_date_time", "=", DateTimeValue{filter.Eq})
		case *UInt32Filter_Ne:
			qb.AddCondition("week_start_date_time", "!=", DateTimeValue{filter.Ne})
		case *UInt32Filter_Lt:
			qb.AddCondition("week_start_date_time", "<", DateTimeValue{filter.Lt})
		case *UInt32Filter_Lte:
			qb.AddCondition("week_start_date_time", "<=", DateTimeValue{filter.Lte})
		case *UInt32Filter_Gt:
			qb.AddCondition("week_start_date_time", ">", DateTimeValue{filter.Gt})
		case *UInt32Filter_Gte:
			qb.AddCondition("week_start_date_time", ">=", DateTimeValue{filter.Gte})
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("week_start_date_time", DateTimeValue{filter.Between.Min}, DateTimeValue{filter.Between.Max.GetValue()})
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				converted := make([]interface{}, len(filter.In.Values))
				for i, v := range filter.In.Values {
				...
				qb.AddInCondition("week_start_date_time", converted)
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				converted := make([]interface{}, len(filter.NotIn.Values))
				...
				qb.AddNotInCondition("week_start_date_time", converted)
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		...
		orderByClause = " ORDER BY week" + ", meta_client_name"
	}
	...
	return BuildParameterizedQuery("fct_block_first_seen_by_node_weekly", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctBlockFirstSeenByNodeWeeklyQuery constructs a parameterized SQL query from a GetFctBlockFirstSeenByNodeWeeklyRequest
func BuildGetFctBlockFirstSeenByNodeWeeklyQuery(req *GetFctBlockFirstSeenByNodeWeeklyRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_block_first_seen_by_node_weekly", columns, qb, orderByClause, 1, 0, options...)
}
