...
// SQL query builder for fct_block_payload_available_by_node
...
// BuildListFctBlockPayloadAvailableByNodeQuery constructs a parameterized SQL query from a ListFctBlockPayloadAvailableByNodeRequest
//
// Available projections:
//   - p_by_slot (primary key: slot)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctBlockPayloadAvailableByNodeQuery(req *ListFctBlockPayloadAvailableByNodeRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: available_slot_start_diff
	if req.AvailableSlotStartDiff != nil {
		switch filter := req.AvailableSlotStartDiff.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("available_slot_start_diff", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("available_slot_start_diff", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("available_slot_start_diff", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("available_slot_start_diff", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("available_slot_start_diff", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("available_slot_start_diff", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("available_slot_start_diff", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("available_slot_start_diff", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("available_slot_start_diff", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "slot", "slot_start_date_time", "epoch", "epoch_start_date_time", "available_slot_start_diff", "block_root", "username", "node_id", "classification", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_consensus_version", "meta_consensus_implementation"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "available_slot_start_diff", "block_root", "username", "node_id", "classification", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_consensus_version", "meta_consensus_implementation"}

	return BuildParameterizedQuery("fct_block_payload_available_by_node", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctBlockPayloadAvailableByNodeQuery constructs a parameterized SQL query from a GetFctBlockPayloadAvailableByNodeRequest
func BuildGetFctBlockPayloadAvailableByNodeQuery(req *GetFctBlockPayloadAvailableByNodeRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_block_payload_available_by_node", columns, qb, orderByClause, 1, 0, options...)
}
