...
// SQL query builder for fct_block_payload_bid
...
// BuildListFctBlockPayloadBidQuery constructs a parameterized SQL query from a ListFctBlockPayloadBidRequest
//
// Available projections:
//   - p_by_slot (primary key: slot)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctBlockPayloadBidQuery(req *ListFctBlockPayloadBidRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.ExecutionPayment != nil {
		...
		case *StringFilter_Contains:
			qb.AddLikeCondition("execution_payment", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("execution_payment", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("execution_payment", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("execution_payment", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("execution_payment", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("execution_payment", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("execution_payment", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "slot", "slot_start_date_time", "epoch", "epoch_start_date_time", "block_root", "block_version", "builder_index", "block_hash", "parent_block_hash", "parent_block_root", "value", "execution_payment", "fee_recipient", "gas_limit", "blob_kzg_commitment_count"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "block_root", "block_version", "builder_index", "NULLIF(`block_hash`, repeat('\x00', 66)) AS `block_hash`", "NULLIF(`parent_block_hash`, repeat('\x00', 66)) AS `parent_block_hash`", "NULLIF(`parent_block_root`, repeat('\x00', 66)) AS `parent_block_root`", "toString(`value`) AS `value`", "toString(`execution_payment`) AS `execution_payment`", "NULLIF(`fee_recipient`, repeat('\x00', 42)) AS `fee_recipient`", "gas_limit", "blob_kzg_commitment_count"}

	return BuildParameterizedQuery("fct_block_payload_bid", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctBlockPayloadBidQuery constructs a parameterized SQL query from a GetFctBlockPayloadBidRequest
func BuildGetFctBlockPayloadBidQuery(req *GetFctBlockPayloadBidRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_block_payload_bid", columns, qb, orderByClause, 1, 0, options...)
}
