...
// SQL query builder for fct_block_payload_ptc_vote_head
...
// BuildListFctBlockPayloadPtcVoteHeadQuery constructs a parameterized SQL query from a ListFctBlockPayloadPtcVoteHeadRequest
//
// Available projections:
//   - p_by_slot (primary key: slot)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctBlockPayloadPtcVoteHeadQuery(req *ListFctBlockPayloadPtcVoteHeadRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: ptc_validators_seen
	if req.PtcValidatorsSeen != nil {
		switch filter := req.PtcValidatorsSeen.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("ptc_validators_seen", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("ptc_validators_seen", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("ptc_validators_seen", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("ptc_validators_seen", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("ptc_validators_seen", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("ptc_validators_seen", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("ptc_validators_seen", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("ptc_validators_seen", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("ptc_validators_seen", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: first_seen_slot_start_diff
	if req.FirstSeenSlotStartDiff != nil {
		switch filter := req.FirstSeenSlotStartDiff.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("first_seen_slot_start_diff", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("first_seen_slot_start_diff", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("first_seen_slot_start_diff", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("first_seen_slot_start_diff", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("first_seen_slot_start_diff", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("first_seen_slot_start_diff", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("first_seen_slot_start_diff", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("first_seen_slot_start_diff", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("first_seen_slot_start_diff", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: last_seen_slot_start_diff
	if req.LastSeenSlotStartDiff != nil {
		switch filter := req.LastSeenSlotStartDiff.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("last_seen_slot_start_diff", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("last_seen_slot_start_diff", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("last_seen_slot_start_diff", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("last_seen_slot_start_diff", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("last_seen_slot_start_diff", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("last_seen_slot_start_diff", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("last_seen_slot_start_diff", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("last_seen_slot_start_diff", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("last_seen_slot_start_diff", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "slot", "slot_start_date_time", "epoch", "epoch_start_date_time", "block_root", "ptc_validators_seen", "payload_present_votes", "blob_data_available_votes", "first_seen_slot_start_diff", "last_seen_slot_start_diff"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "block_root", "ptc_validators_seen", "payload_present_votes", "blob_data_available_votes", "first_seen_slot_start_diff", "last_seen_slot_start_diff"}

	return BuildParameterizedQuery("fct_block_payload_ptc_vote_head", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctBlockPayloadPtcVoteHeadQuery constructs a parameterized SQL query from a GetFctBlockPayloadPtcVoteHeadRequest
func BuildGetFctBlockPayloadPtcVoteHeadQuery(req *GetFctBlockPayloadPtcVoteHeadRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_block_payload_ptc_vote_head", columns, qb, orderByClause, 1, 0, options...)
}
