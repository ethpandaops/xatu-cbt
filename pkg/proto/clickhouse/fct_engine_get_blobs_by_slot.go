...
// SQL query builder for fct_engine_get_blobs_by_slot
...
// BuildListFctEngineGetBlobsBySlotQuery constructs a parameterized SQL query from a ListFctEngineGetBlobsBySlotRequest
func BuildListFctEngineGetBlobsBySlotQuery(req *ListFctEngineGetBlobsBySlotRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: max_requested_count
	if req.MaxRequestedCount != nil {
		switch filter := req.MaxRequestedCount.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("max_requested_count", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("max_requested_count", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("max_requested_count", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("max_requested_count", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("max_requested_count", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("max_requested_count", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("max_requested_count", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("max_requested_count", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("max_requested_count", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "slot", "slot_start_date_time", "epoch", "epoch_start_date_time", "block_root", "is_reference_node", "observation_count", "unique_node_count", "max_requested_count", "avg_returned_count", "success_count", "partial_count", "empty_count", "unsupported_count", "error_count", "success_pct", "full_return_pct", "avg_duration_ms", "median_duration_ms", "min_duration_ms", "max_duration_ms", "p95_duration_ms", "unique_cl_implementation_count", "unique_el_implementation_count"}
		...
		orderByClause = " ORDER BY slot_start_date_time" + ", block_root" + ", is_reference_node"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "NULLIF(`block_root`, repeat('\x00', 66)) AS `block_root`", "is_reference_node", "observation_count", "unique_node_count", "max_requested_count", "avg_returned_count", "success_count", "partial_count", "empty_count", "unsupported_count", "error_count", "success_pct", "full_return_pct", "avg_duration_ms", "median_duration_ms", "min_duration_ms", "max_duration_ms", "p95_duration_ms", "unique_cl_implementation_count", "unique_el_implementation_count"}

	return BuildParameterizedQuery("fct_engine_get_blobs_by_slot", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctEngineGetBlobsBySlotQuery constructs a parameterized SQL query from a GetFctEngineGetBlobsBySlotRequest
func BuildGetFctEngineGetBlobsBySlotQuery(req *GetFctEngineGetBlobsBySlotRequest, options ...QueryOption) (SQLQuery, error) {
