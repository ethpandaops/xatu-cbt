...
// SQL query builder for fct_execution_state_size_hourly
...
// BuildListFctExecutionStateSizeHourlyQuery constructs a parameterized SQL query from a ListFctExecutionStateSizeHourlyRequest
func BuildListFctExecutionStateSizeHourlyQuery(req *ListFctExecutionStateSizeHourlyRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: measurement_count
	if req.MeasurementCount != nil {
		switch filter := req.MeasurementCount.Filter.(type) {
		case *UInt32Filter_Eq:
			qb.AddCondition("measurement_count", "=", filter.Eq)
		case *UInt32Filter_Ne:
			qb.AddCondition("measurement_count", "!=", filter.Ne)
		case *UInt32Filter_Lt:
			qb.AddCondition("measurement_count", "<", filter.Lt)
		case *UInt32Filter_Lte:
			qb.AddCondition("measurement_count", "<=", filter.Lte)
		case *UInt32Filter_Gt:
			qb.AddCondition("measurement_count", ">", filter.Gt)
		case *UInt32Filter_Gte:
			qb.AddCondition("measurement_count", ">=", filter.Gte)
		case *UInt32Filter_Between:
			qb.AddBetweenCondition("measurement_count", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("measurement_count", UInt32SliceToInterface(filter.In.Values))
			}
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("measurement_count", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp(`hour_start_date_time`) AS `hour_start_date_time`", "measurement_count", "min_block_number", "max_block_number", "accounts", "account_bytes", "account_trienodes", "account_trienode_bytes", "contract_codes", "contract_code_bytes", "storages", "storage_bytes", "storage_trienodes", "storage_trienode_bytes", "accounts_delta", "account_bytes_delta", "storage_bytes_delta", "contract_code_bytes_delta", "total_bytes"}

	return BuildParameterizedQuery("fct_execution_state_size_hourly", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctExecutionStateSizeHourlyQuery constructs a parameterized SQL query from a GetFctExecutionStateSizeHourlyRequest
func BuildGetFctExecutionStateSizeHourlyQuery(req *GetFctExecutionStateSizeHourlyRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_execution_state_size_hourly", columns, qb, orderByClause, 1, 0, options...)
}
