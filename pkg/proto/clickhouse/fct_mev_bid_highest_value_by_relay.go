...
// SQL query builder for fct_mev_bid_highest_value_by_relay
...
// BuildListFctMevBidHighestValueByRelayQuery constructs a parameterized SQL query from a ListFctMevBidHighestValueByRelayRequest
//
// Available projections:
//   - p_by_block_hash (primary key: block_hash)
//   - p_by_slot (primary key: slot)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctMevBidHighestValueByRelayQuery(req *ListFctMevBidHighestValueByRelayRequest, options ...QueryOption) (SQLQuery, error) {
	// Validate that at least one primary key is provided
	// Primary keys can come from base table or projections
	if req.BlockHash == nil && req.Slot == nil && req.SlotStartDateTime == nil {
		return SQLQuery{}, fmt.Errorf("at least one primary key field is required: block_hash, slot, slot_start_date_time")
	}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "slot", "slot_start_date_time", "epoch", "epoch_start_date_time", "bid_date_time", "relay_name", "parent_hash", "block_number", "block_hash", "builder_pubkey", "proposer_pubkey", "proposer_fee_recipient", "gas_limit", "gas_used", "value", "transaction_count", "optimistic_submission"}
	...
	return BuildParameterizedQuery("fct_mev_bid_highest_value_by_relay", qb, orderByClause, limit, offset, options...), nil
}

// BuildGetFctMevBidHighestValueByRelayQuery constructs a parameterized SQL query from a GetFctMevBidHighestValueByRelayRequest
func BuildGetFctMevBidHighestValueByRelayQuery(req *GetFctMevBidHighestValueByRelayRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_mev_bid_highest_value_by_relay", qb, orderByClause, 1, 0, options...), nil
}
