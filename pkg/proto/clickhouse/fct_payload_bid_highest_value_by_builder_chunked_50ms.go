...
// SQL query builder for fct_payload_bid_highest_value_by_builder_chunked_50ms
...
// BuildListFctPayloadBidHighestValueByBuilderChunked50MsQuery constructs a parameterized SQL query from a ListFctPayloadBidHighestValueByBuilderChunked50MsRequest
//
// Available projections:
//   - p_by_slot (primary key: slot)
//
// Use WithProjection() option to select a specific projection.
func BuildListFctPayloadBidHighestValueByBuilderChunked50MsQuery(req *ListFctPayloadBidHighestValueByBuilderChunked50MsRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "slot", "slot_start_date_time", "epoch", "epoch_start_date_time", "chunk_slot_start_diff", "earliest_bid_date_time", "block_hash", "builder_index", "value", "execution_payment", "fee_recipient"}
		...
		orderByClause = " ORDER BY slot_start_date_time" + ", chunk_slot_start_diff" + ", builder_index"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "chunk_slot_start_diff", "toUnixTimestamp64Micro(`earliest_bid_date_time`) AS `earliest_bid_date_time`", "NULLIF(`block_hash`, repeat('\x00', 66)) AS `block_hash`", "builder_index", "toString(`value`) AS `value`", "toString(`execution_payment`) AS `execution_payment`", "NULLIF(`fee_recipient`, repeat('\x00', 42)) AS `fee_recipient`"}

	return BuildParameterizedQuery("fct_payload_bid_highest_value_by_builder_chunked_50ms", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetFctPayloadBidHighestValueByBuilderChunked50MsQuery constructs a parameterized SQL query from a GetFctPayloadBidHighestValueByBuilderChunked50MsRequest
func BuildGetFctPayloadBidHighestValueByBuilderChunked50MsQuery(req *GetFctPayloadBidHighestValueByBuilderChunked50MsRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("fct_payload_bid_highest_value_by_builder_chunked_50ms", columns, qb, orderByClause, 1, 0, options...)
}
