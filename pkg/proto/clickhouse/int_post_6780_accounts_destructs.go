...
// SQL query builder for int_post_6780_accounts_destructs
...
// BuildListIntPost6780AccountsDestructsQuery constructs a parameterized SQL query from a ListIntPost6780AccountsDestructsRequest
func BuildListIntPost6780AccountsDestructsQuery(req *ListIntPost6780AccountsDestructsRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: is_same_tx
	if req.IsSameTx != nil {
		switch filter := req.IsSameTx.Filter.(type) {
		case *BoolFilter_Eq:
			qb.AddCondition("is_same_tx", "=", filter.Eq)
		case *BoolFilter_Ne:
			qb.AddCondition("is_same_tx", "!=", filter.Ne)
		default:
			// Unsupported filter type
		}
	}
	...
	if req.OrderBy != "" {
		validFields := []string{"address", "block_number", "transaction_hash", "transaction_index", "is_same_tx"}
	...
	// Build column list
	columns := []string{"address", "block_number", "NULLIF(`transaction_hash`, repeat('\x00', 66)) AS `transaction_hash`", "transaction_index", "is_same_tx"}

	return BuildParameterizedQuery("int_post_6780_accounts_destructs", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetIntPost6780AccountsDestructsQuery constructs a parameterized SQL query from a GetIntPost6780AccountsDestructsRequest
func BuildGetIntPost6780AccountsDestructsQuery(req *GetIntPost6780AccountsDestructsRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("int_post_6780_accounts_destructs", columns, qb, orderByClause, 1, 0, options...)
}
