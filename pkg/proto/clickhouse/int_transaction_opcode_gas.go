...
func BuildListIntTransactionOpcodeGasQuery(req *ListIntTransactionOpcodeGasRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "block_number", "transaction_hash", "transaction_index", "opcode", "count", "total_gas", "min_depth", "max_depth", "error_count", "meta_network_name"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "block_number", "NULLIF(`transaction_hash`, repeat('\x00', 66)) AS `transaction_hash`", "transaction_index", "opcode", "count", "total_gas", "min_depth", "max_depth", "error_count", "meta_network_name"}
